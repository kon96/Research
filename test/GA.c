...
const char *DATADIR = "C:\\Users\\owner\\Desktop\\test\\ALL_tsp";
int    NodeNum;                             // ノード数
...
int main(int argc, char *argv[]) {
    ...
    if (!(fp = fopenex("C:\\Users\\owner\\Desktop\\test\\ALL_tsp\\tsp_opt.txt", "r"))) return 1;
    while (fgets(linebuf, sizeof(linebuf), fp) != NULL) {
        if (sscanf(linebuf, "%s %d", buf, &length) < 2) {
            printf("%s  %s  %d\n",linebuf,buf,length);
            break;
        }
